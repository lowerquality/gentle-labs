...
pwd
mkdir -p $dir/log
[[ -d $sdata && $data/feats.scp -ot $sdata ]] || ./steps/split_data.sh $data $nj || exit 1;
...
echo $sdata/feats.scp;
echo $graphdir;
pwd

#for f in $sdata/1/feats.scp $sdata/1/cmvn.scp $model $graphdir/HCLG.fst; do
for f in $sdata/feats.scp $sdata/cmvn.scp $model $graphdir/HCLG.fst; do
...
case $feat_type in
  delta) feats="ark,s,cs:apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/utt2spk scp:$sdata/cmvn.scp scp,p:$sdata/feats.scp ark:- | add-deltas $delta_opts ark:- ark:- |";;
  lda) feats="ark,s,cs:apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/utt2spk scp:$sdata/cmvn.scp scp:$sdata/feats.scp ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $srcdir/final.mat ark:- ark:- |";;
  *) echo "$0: Error: Invalid feature type $feat_type" && exit 1;
esac

if [ ! -z "$transform_dir" ]; then # add transforms to features...  echo "Using fMLLR transforms from $transform_dir"
  ...
  if [ $nj -ne $nj_orig ]; then
    ...
    feats="$feats transform-feats --utt2spk=ark:$sdata/utt2spk scp:$dir/trans.scp ark:- ark:- |"
  else
    # number of jobs matches with alignment dir.
    feats="$feats transform-feats --utt2spk=ark:$sdata/utt2spk ark:$transform_dir/trans.JOB ark:- ark:- |"
  fi
fi

if [ $stage -le 0 ]; then
  if [ -f "$graphdir/num_pdfs" ]; then
    [ "`cat $graphdir/num_pdfs`" -eq `am-info --print-args=true $model | grep pdfs | awk '{print $NF}'` ] || \
  ...
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode.JOB.log \
    ...
    # $model $graphdir/HCLG.fst "$feats" "ark:|gzip -c > $dir/lat.JOB.gz" || exit 1;
       ...
       # Can give a `words_wspecifier` and an `alignment_wspecifier` as arguments to `gmm-latgen-faster`
